...
void APVZ_USFX_LAB02GameMode::BeginPlay()
{
	...
	// Creamos los objetos de las hordas
	AHordaFactory* HordaFacil = GetWorld()->SpawnActor<AHordaFacilFactory>(AHordaFacilFactory::StaticClass());
	AHordaFactory* HordaMedia = GetWorld()->SpawnActor<AHordaMediaFactory>(AHordaMediaFactory::StaticClass());
	AHordaFactory* HordaDificil = GetWorld()->SpawnActor<AHordaDificilFactory>(AHordaDificilFactory::StaticClass());

// Creamos la horda
	for (int32 i = 0; i < 5; ++i)
	{
		ArrayHordaZombies.Add(HordaMedia->crearHorda(i));
	}
